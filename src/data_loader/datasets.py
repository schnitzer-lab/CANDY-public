...
class DataTraces(Dataset):
    ...
    def __init__(self, traces_trials_t, traces_trials_tpn, trial_sections, trial_types):
        """
        Args:
        ...
        """
        if not isinstance(traces_trials_t, torch.Tensor):
            self.Xt   = torch.tensor(traces_trials_t, dtype=torch.float32)
            self.Xtpn = torch.tensor(traces_trials_tpn, dtype=torch.float32)
        else:
            self.Xt   = traces_trials_t.type(torch.FloatTensor)
            self.Xtpn = traces_trials_tpn.type(torch.FloatTensor)
        self.trial_sections = np.array(trial_sections)
        self.trial_types    = np.array(trial_types)

    def __len__(self):
        return len(self.Xt)

    def __getitem__(self, idx):
        return self.Xt[idx], self.Xtpn[idx], self.trial_sections[idx]

def load_rnndataset(dataset):
    """
    INPUT
