...
def _wheelspeed2lego(y):
    """
    Convert the wheel speed to lego pieces.
    """
    Lego_d = 6.24
    ppr    = 1024
    Lego_l = Lego_d * np.pi 
    cm_per_click = Lego_l / ppr 
    y_lego = y / cm_per_click
    return y_lego

def _wheelspeed2cmpsec(y):
    """
    Convert the wheel speed to cm/sec
    """
    Lego_d = 6.24
    ppr    = 1024
    Lego_l = Lego_d * np.pi 
    cm_per_click = Lego_l / ppr 
    y_cmpsec = y * cm_per_click
    return y_cmpsec

def _get_speedclass(y):
    """
    Convert the speed to 5 classes
    """
    y_temp = np.maximum(y, -1024)
    y_temp = np.minimum(y_temp, 1024)
    y_class = np.round(y_temp / 512)
